...
import ZipFileLoader from '../storage/ZipFileLoader';
import UnpackedDirLoader from '../storage/UnpackedDirLoader';
...
LibraryWriter.prototype = {
  ...
  _addEpub: function(options, packageObj, packagePath, encryptionData) {
    ...
    var rootDirName =
        this.rootDirName || new Date().getTime() + '' + Math.floor(Math.random() * 1000),
      self = this,
    ...
    var commitEpubToLibrary = function() {
      ...
      if (packageObj.coverHref) {
        ...
        epubObj.coverHref = StorageManager.getPathUrl(
          rootDirName + '/' + self._getFullUrl(packagePath, packageObj.coverHref),
        );
      }
    ...
    var max = fileLoader.loadAllFiles(['META-INF/encryption.xml'], startAtIndex, function(
      continueCallback,
      name,
      file,
    ) {
      var callback = function(name, transformedContent) {
        ...
        StorageManager.saveFile(
          ...
          function() {
            options.progress(
              Math.round(100 * (++count / max)),
              Messages.PROGRESS_WRITING,
              count + ' of ' + max + ' (' + name + ')',
            );
    ...
    this._loadFileAsString('META-INF/container.xml', function(containerStr) {
      self._findPackagePath(containerStr, function(packagePath) {
        self._loadFileAsString(packagePath, function(packageStr) {
          self._loadFileAsString(
            'META-INF/encryption.xml',
            function(encryptionStr) {
              self._parsePackageData(packageStr, encryptionStr, function(
                packageObj,
                encryptionData,
              ) {
  ...
  importUrl: function(url, callbacks) {
    ...
    (self = this),
      (error = function() {
        callbacks.error(Messages.ERROR_AJAX);
      });
...
onmessage = function(evt) {
  ...
  var continueImport = function(buf, index, rootDirName) {
    postMessage({
      msg: Messages.CONTINUE_IMPORT_ZIP,
      buf: buf,
      index: index,
      rootDirName: rootDirName,
      libraryItems: writer.libraryData,
    });
  };
  ...
  switch (msg) {
    ...
    case Messages.CONTINUE_IMPORT_ZIP:
      ...
      StorageManager.initStorage(function() {
        writer.continueImportZip(buf, index, rootDirName, {
          success: success,
          progress: progress,
          error: error,
          overwrite: overwrite,
