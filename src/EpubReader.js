...
import AboutDialog from '../resources/templates/about-dialog.html';
import ReaderNavbar from '../resources/templates/reader-navbar.html';
import ReaderBody from '../resources/templates/reader-body.html';
import ReaderBodyPageButtons from '../resources/templates/reader-body-page-btns.html';
...
import Versioning from './versioning/ReadiumVersioning';
...
var ensureUrlIsRelativeToApp = function(ebookURL) {
  ...
  if (appUrl) {
    ...
    if (ebookURL.indexOf('//') == 0) {
      // URI.relativeTo() sometimes returns "//domain.com/path" without the protocol
...
var loadEbook = function(readerSettings, openPageRequest) {
  readium.openPackageDocument(
    ...
    function(packageDocument, options) {
      ...
      var wasFixed = readium.reader.isCurrentViewFixedLayout();
...
var loadToc = function(dom) {
  if (dom) {
    ...
    while (pparent && pparent.getAttributeNS) {
      var dir =
        pparent.getAttributeNS('http://www.w3.org/1999/xhtml', 'dir') ||
        pparent.getAttribute('dir');
...
var updateUI = function(pageChangeData) {
  if (pageChangeData.paginationInfo.canGoLeft()) $('#left-page-btn').show();
  else $('#left-page-btn').hide();
  if (pageChangeData.paginationInfo.canGoRight()) $('#right-page-btn').show();
  else $('#right-page-btn').hide();
};
...
var savePlace = function() {
  ...
  if (
    !isChromeExtensionPackagedApp && // History API is disabled in packaged apps
    window.history &&
...
var installReaderEventHandlers = function() {
  if (isChromeExtensionPackagedApp) {
    ...
    $('.icon-shareUrl').on('click', function() {
      ...
      if (readium.reader.plugins.highlights) {
        ...
        //BookmarkData
        var bookmarkDataSelection = readium.reader.plugins.highlights.addSelectionHighlight(
          tempId,
          'temp-highlight',
        );
      ...
      try {
        ...
        if (coverHref) {
          ...
          if (fetcher.shouldConstructDomProgrammatically()) {
            fetcher.relativeToPackageFetchFileContents(
              relPath,
              'blob',
              function(res) {
                if (res) {
  ...
  $('.icon-annotations').on('click', function() {
    readium.reader.plugins.highlights.addSelectionHighlight(
...
var loadReaderUI = function(data) {
  ...
  if (window.ReadiumSDK) {
    ...
    Globals.off(Globals.Events.PLUGINS_LOADED);
  }
...
var initReadium = function() {
  ...
  Settings.getMultiple(['reader', ebookURL_filepath], function(settings) {
    ...
    Versioning.getVersioningInfo(function(version) {
      ...
      //
      // var repo3 = {};
      // repo3.name = '@evidentpoint/readium-shared-js';
