...
import SettingsDialog from '../resources/templates/settings-dialog.html';
...
var initDialog = function(reader) {
  ...
  $columnMaxWidthSlider.on('change', function() {
    ...
    Settings.get('reader', function(readerSettings) {
      ...
      for (var prop in defaultSettings) {
        if (
          ...
          (!readerSettings.hasOwnProperty(prop) || typeof readerSettings[prop] == 'undefined')
        ) {
      ...
      if ($fontSelectionList[0].childElementCount == 1) {
        ...
        $.each(moduleConfig.fonts, function(index, fontObj) {
          ...
          if (fontObj.url) {
            //No url, no problem so long as there's a font that works.
            var fontPayload =
              '<link id="fontStyle_' +
              index +
      ...
      var columnMaxWidth_text =
        columnMaxWidth >= maxVal ? Strings.i18n_pageMaxWidth_Disabled : columnMaxWidth + 'px';
      $columnMaxWidthSlider.val(columnMaxWidth);
      updateSliderLabels(
        $columnMaxWidthSlider,
        columnMaxWidth,
        columnMaxWidth_text,
        Strings.i18n_pageMaxWidth,
      );
